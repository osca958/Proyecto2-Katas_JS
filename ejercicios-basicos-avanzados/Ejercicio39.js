...
let contador = {}; // creo un objeto vacio para ir guardando cada sonido con su cantidad

for (const user of users) {
    for (const sonido in user.favoritesSounds){
    if (contador[sonido]){ // si existe la clave sonido en contador
        contador[sonido]+=1; //le añado uno
    } else {
        contador[sonido] = 1; // sino lo inicializo a 1
    }
    }
}
